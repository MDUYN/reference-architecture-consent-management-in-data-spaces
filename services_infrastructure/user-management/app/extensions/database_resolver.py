...
class SQLAlchemyDatabaseResolver:
    ...
    def set_postgresql_config(self, config) -> None:
        ...
        try:
            self.config[DATABASE_URL] =  'postgresql://{}:{}@{}/{}'.format(
                config[DATABASE_USERNAME], config[DATABASE_PASSWORD], config[DATABASE_HOST], config[DATABASE_NAME]
            )
    ...
    def configure(self, database_config: dict = None, ssl_require: bool = False) -> None:
